...
class VerticalDeckView @JvmOverloads constructor(
    ...
    private val kinematics = SpindleKinematics(hubRadiusRatio = 0.50f, maxTapeRadiusRatio = 0.825f)
    ...
    var onHoldSeekStart: ((isForward: Boolean) -> Unit)? = null
    var onHoldSeekProgress: ((pitch: Float) -> Unit)? = null
    ...
    private val chassisGestureDetector by lazy {
        GestureDetector(context, object : GestureDetector.SimpleOnGestureListener() {
            ...
            override fun onSingleTapConfirmed(e: MotionEvent): Boolean {
                if (isTouchInsideButtonsOrProgress(e.x, e.y)) {
                    return false
                }
    ...
    private fun isTouchInsideButtonsOrProgress(x: Float, y: Float): Boolean {
        val slop = 12f
        val inTape = x >= tapeTypeBadgeRect.left - slop && x <= tapeTypeBadgeRect.right + slop &&
                     y >= tapeTypeBadgeRect.top - slop && y <= tapeTypeBadgeRect.bottom + slop
        val inDolby = x >= dolbyBadgeRect.left - slop && x <= dolbyBadgeRect.right + slop &&
    ...
    private val leaderTapePaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        color = Color.parseColor("#40FFFFFF") // Clear translucent leader ribbon
        style = Paint.Style.STROKE
        strokeWidth = 7.5f
    }
    private val leaderTapeHighlightPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        color = Color.parseColor("#70FFFFFF")
        style = Paint.Style.STROKE
        strokeWidth = 1.5f
    }
    private val leaderSpoolRingPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        color = Color.parseColor("#45FFFFFF")
        style = Paint.Style.STROKE
        strokeWidth = 3.5f
    }
    private val tapeSplicePaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        color = Color.parseColor("#F59E0B") // Vintage amber/yellow splice adhesive
    ...
    // Dedicated Paints for Two Small Precision Guide Rollers
    private val guideRollerBodyPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply { style = Paint.Style.FILL }
    private val guideRollerRimPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply { style = Paint.Style.STROKE; strokeWidth = 1.6f }
    private val guideRollerPinPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply { style = Paint.Style.FILL }
    private val guideRollerWellPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply { style = Paint.Style.FILL }
    private val guideRollerStrobePaint = Paint(Paint.ANTI_ALIAS_FLAG).apply { style = Paint.Style.FILL }

    // Dedicated Paints for Mumetal Magnetic Shield Plate behind felt pad & tape path
    private val magneticShieldPlatePaint = Paint(Paint.ANTI_ALIAS_FLAG).apply { style = Paint.Style.FILL }
    private val magneticShieldBorderPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply { style = Paint.Style.STROKE; strokeWidth = 1.2f }
    private val magneticShieldRect = RectF()

    // Tactile Cassette Tape Formulation & Dolby Noise Reduction Selectors
    ...
    private val tapeBadgeSubTextPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        typeface = Typeface.create(Typeface.SANS_SERIF, Typeface.BOLD)
        textAlign = Paint.Align.CENTER
    }

    var tapeFormulation: com.hana.spindle.playback.AudioFxController.TapeFormulation =
    ...
    var dolbyMode: com.hana.spindle.playback.AudioFxController.DolbyMode =
        com.hana.spindle.playback.AudioFxController.DolbyMode.OFF
        set(value) {
            if (field != value) {
                field = value
                invalidate()
    ...
    private val topTangentPoints = FloatArray(4)
    private val bottomTangentPoints = FloatArray(4)

    // Expansive Studio Paper Label Geometry & Slit Glares
    ...
    private val albumArtClipPath = Path()
    private val albumArtBorderPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        style = Paint.Style.STROKE
        strokeWidth = 1.2f
        color = Color.argb(40, 0, 0, 0)
    }
    private val albumArtPlaceholderPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        style = Paint.Style.FILL
        color = Color.parseColor("#1E2132")
    }
    private val albumArtIconPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        style = Paint.Style.FILL
    ...
    private fun updatePaints() {
        ...
        if (isEink) {
            ...
            tapeShellSpoolPaint.apply { color = Color.argb(180, 0, 0, 0); style = Paint.Style.FILL }
            ...
            tapePathPaint.apply { color = Color.BLACK; style = Paint.Style.STROKE; strokeWidth = 7.5f }
            ...
            tapeSpoolPaint.apply { color = Color.parseColor("#4A2E20"); style = Paint.Style.FILL }
            tapeShellSpoolPaint.apply { color = Color.argb(195, 80, 50, 40); style = Paint.Style.FILL }
            ...
            tapePathPaint.apply { color = Color.parseColor("#5A3624"); style = Paint.Style.STROKE; strokeWidth = 7.5f }
            tapePathHighlightPaint.apply { color = Color.argb(130, 200, 150, 120); style = Paint.Style.STROKE; strokeWidth = 1.6f }
            ...
            tapeSpoolPaint.apply { color = Color.parseColor("#3D241A"); style = Paint.Style.FILL }
            tapeShellSpoolPaint.apply { color = Color.argb(195, 65, 38, 28); style = Paint.Style.FILL }
            tapeTexturePaint.apply { color = Color.argb(45, 180, 140, 120); style = Paint.Style.STROKE; strokeWidth = 1.2f }
            ...
            tapePathPaint.apply { color = Color.parseColor("#4E2F20"); style = Paint.Style.STROKE; strokeWidth = 7.5f }
            tapePathHighlightPaint.apply { color = Color.argb(130, 160, 110, 85); style = Paint.Style.STROKE; strokeWidth = 1.6f }
    ...
    private fun updateAccentPaints() {
        ...
        if (isEink) {
            ...
            labelStripePaint.color = Color.BLACK
            labelSideBadgePaint.color = Color.BLACK
            labelTitleTextPaint.color = Color.BLACK
            labelSubTextPaint.color = Color.BLACK
            labelSpecsTextPaint.color = Color.BLACK
            rulerTickPaint.color = Color.BLACK
            rulerTextPaint.color = Color.BLACK
            innerBevelPaint.color = Color.BLACK
            albumArtBorderPaint.color = Color.BLACK
            albumArtPlaceholderPaint.color = Color.WHITE
            albumArtIconPaint.color = Color.BLACK
            return
        }
        ...
        // Expansive Studio Paper Label colors (Matching LiteDeckView exactly)
        val isMetal = theme.name.contains("METAL", ignoreCase = true) || theme.id.contains("metal", ignoreCase = true)
        val isType1 = !isMetal && (theme.name.contains("NORMAL", ignoreCase = true) || (theme.name.contains("TYPE I", ignoreCase = true) && !theme.name.contains("TYPE II", ignoreCase = true)))
        if (isMetal) {
            labelPaint.color = Color.parseColor("#FACC15")
            labelTitleTextPaint.color = Color.parseColor("#0F172A")
            labelStripePaint.color = Color.parseColor("#F97316")
            labelSideBadgePaint.color = Color.parseColor("#EF4444")
            labelSubTextPaint.color = Color.argb(135, 0, 0, 0)
            labelSpecsTextPaint.color = Color.argb(165, 120, 53, 15)
            labelRuledLinePaint.color = Color.argb(45, 0, 0, 0)
            rulerTickPaint.color = Color.argb(120, 253, 230, 138)
            rulerTextPaint.color = Color.argb(140, 253, 230, 138)
            innerBevelPaint.color = Color.argb(55, 255, 255, 255)
        } else if (isType1) {
            labelPaint.color = Color.parseColor("#E2E8F0")
            labelTitleTextPaint.color = Color.parseColor("#1E293B")
            labelStripePaint.color = Color.parseColor("#3B82F6")
            labelSideBadgePaint.color = Color.parseColor("#3B82F6")
            labelSubTextPaint.color = Color.argb(135, 0, 0, 0)
            labelSpecsTextPaint.color = Color.argb(165, 30, 58, 138)
            labelRuledLinePaint.color = Color.argb(45, 0, 0, 0)
            rulerTickPaint.color = Color.argb(120, 253, 230, 138)
            rulerTextPaint.color = Color.argb(140, 253, 230, 138)
            innerBevelPaint.color = Color.argb(45, 255, 255, 255)
        } else {
            // Default Type II Chrome Hi-Bias / Studio
            labelPaint.color = Color.parseColor("#FDE68A")
            labelTitleTextPaint.color = Color.parseColor("#1E2132")
            labelStripePaint.color = Color.parseColor("#F97316")
            labelSideBadgePaint.color = Color.parseColor("#F97316")
            labelSubTextPaint.color = Color.argb(135, 0, 0, 0)
            labelSpecsTextPaint.color = Color.argb(165, 120, 53, 15)
            labelRuledLinePaint.color = Color.argb(45, 0, 0, 0)
            rulerTickPaint.color = Color.argb(120, 253, 230, 138)
        ...
        albumArtBorderPaint.color = if (isLight) Color.argb(45, 42, 46, 69) else Color.argb(60, 0, 0, 0)
        albumArtPlaceholderPaint.color = if (isLight) Color.parseColor("#E5E5E2") else Color.parseColor("#181A27")
        albumArtIconPaint.color = accent

        // 2 Small Guide Rollers & Mumetal Magnetic Shielding Plate
        if (isEink) {
            guideRollerBodyPaint.color = Color.WHITE
            guideRollerRimPaint.color = Color.BLACK
            guideRollerPinPaint.color = Color.BLACK
            guideRollerWellPaint.color = Color.WHITE
            guideRollerStrobePaint.color = Color.BLACK

            magneticShieldPlatePaint.color = Color.WHITE
            magneticShieldBorderPaint.color = Color.BLACK

            tapeBadgeBgPaint.color = Color.WHITE
            tapeBadgeBorderPaint.color = Color.BLACK
            tapeBadgeTextPaint.color = Color.BLACK
            tapeBadgeSubTextPaint.color = Color.BLACK
        } else if (isLight) {
            guideRollerBodyPaint.color = Color.parseColor("#F1F5F9")
            guideRollerRimPaint.color = Color.parseColor("#94A3B8")
            guideRollerPinPaint.color = Color.parseColor("#475569")
            guideRollerWellPaint.color = Color.parseColor("#E2E8F0")
            guideRollerStrobePaint.color = Color.parseColor("#F97316")

            magneticShieldPlatePaint.color = Color.parseColor("#CBD5E1")
            magneticShieldBorderPaint.color = Color.parseColor("#64748B")

            tapeBadgeBgPaint.color = Color.argb(180, 241, 245, 249)
            tapeBadgeBorderPaint.color = Color.parseColor("#94A3B8")
            tapeBadgeTextPaint.color = Color.parseColor("#1E293B")
            tapeBadgeSubTextPaint.color = Color.parseColor("#64748B")
        } else {
            guideRollerBodyPaint.color = Color.parseColor("#F8FAFC")
            guideRollerRimPaint.color = Color.parseColor("#474B66")
            guideRollerPinPaint.color = Color.parseColor("#CBD5E1")
            guideRollerWellPaint.color = Color.parseColor("#141622")
            guideRollerStrobePaint.color = accent

            magneticShieldPlatePaint.color = Color.parseColor("#383C56")
            magneticShieldBorderPaint.color = Color.parseColor("#64748B")

            tapeBadgeBgPaint.color = Color.argb(160, 26, 29, 44)
            tapeBadgeBorderPaint.color = Color.parseColor("#474F6E")
            tapeBadgeTextPaint.color = Color.parseColor("#FAFAF9")
            tapeBadgeSubTextPaint.color = Color.parseColor("#94A3B8")
        }

        fitStudioLabelText()
    }

    private fun getFormulationTag(): String = tapeFormulation.tag

    private fun formatPlaybackTime(currentMs: Long, durationMs: Long): String {
        val curM = TimeUnit.MILLISECONDS.toMinutes(currentMs)
        val curS = TimeUnit.MILLISECONDS.toSeconds(currentMs) % 60
        val durM = TimeUnit.MILLISECONDS.toMinutes(durationMs)
        val durS = TimeUnit.MILLISECONDS.toSeconds(durationMs) % 60
        return String.format(Locale.US, "%02d:%02d / %02d:%02d", curM, curS, durM, durS)
    }

    private fun fitStudioLabelText() {
        val maxLabelTextWidth = (labelRect.bottom - labelSideBadgeRect.bottom) * 0.84f
        val lw = labelRect.width()
        if (maxLabelTextWidth <= 0f || lw <= 0f) return

        // Line 1: Track Title (top)
        val displayTitle = if (isSongPresent()) trackTitle else if (deviceName.isNotBlank()) deviceName else com.hana.spindle.util.DeviceUtils.getHardwareDeviceName()
        ...
        if (!isPlaying && measuredTitle > maxLabelTextWidth && measuredTitle > 0f) {
        ...
        val displayArtistTime = if (isSongPresent()) {
            val timeStr = formatPlaybackTime(currentTimeMs, durationMs)
            if (artistName.isNotBlank() && artistName != "Unknown Artist") "$artistName • $timeStr" else timeStr
        } else {
        ...
        val displaySpecs = if (isSongPresent()) {
            val specs = if (audioFormat.isNotBlank()) audioFormat else "HI-RES AUDIO"
            val route = getRouteRowText()
            "$specs • $route"
        } else {
            if (androidVersionText.isNotBlank()) androidVersionText else com.hana.spindle.util.DeviceUtils.getAndroidVersionString()
        }
    ...
    override fun onSizeChanged(w: Int, h: Int, oldw: Int, oldh: Int) {
        ...
        // 3. Status Row directly above 12-LED Progress Bar:
        // - Left: Interactive Selector Badges (Tape Formulation & Dolby NR)
        // - Right: Battery and Play Status (Run) LEDs
        val statusRowH = (h * 0.026f).coerceAtLeast(barH * 2.0f)
        val statusRowGap = h * 0.014f
        val statusRowBottom = barTop - statusRowGap
        val statusRowTop = statusRowBottom - statusRowH
        ...
        val badge2Left = tapeTypeBadgeRect.right + badgeGap
        dolbyBadgeRect.set(badge2Left, statusRowTop, badge2Left + badgeW2, statusRowBottom)
        ...
        // Right side: Battery & Run Status LEDs aligned to right (ledBarRect.right)
        val statusLedH = barH
        val statusLedW = w * 0.075f
        ...
        val labelPillGap = w * 0.012f
        val centerClusterGap = w * 0.032f

        val statusLedTop = statusRowMidY - statusLedH * 0.5f
        val statusLedBottom = statusLedTop + statusLedH
        val labelY = statusRowMidY - ((statusLabelPaint.descent() + statusLabelPaint.ascent()) * 0.5f)

        // Cluster ending at ledBarRect.right: [Battery label] [battery LED] [Run LED] [Run label]
        val activeLabelRight = ledBarRect.right
        activeLabelX = activeLabelRight - activeTextW
        activeLabelY = labelY

        val activeLedRight = activeLabelX - labelPillGap
        val activeLedLeft = activeLedRight - statusLedW
        ledRunRect.set(activeLedLeft, statusLedTop, activeLedRight, statusLedBottom)

        val batteryLedRight = activeLedLeft - centerClusterGap
        val batteryLedLeft = batteryLedRight - statusLedW
        ledPeakRect.set(batteryLedLeft, statusLedTop, batteryLedRight, statusLedBottom)

        val batteryLabelRight = batteryLedLeft - labelPillGap
        batteryLabelX = batteryLabelRight - batteryTextW
        batteryLabelY = labelY

        // 4. Cassette FULL HEIGHT: fills entire space above the status row ("cassette full height")
        val slotInset = w * 0.008f
        val cTop = h * 0.020f
        val cBottom = statusRowTop - h * 0.012f - slotInset
        val ch = cBottom - cTop
        ...
        val hubRadius = cw * 0.122f
        hubOuterRadius = hubRadius
        baseHubDimension = hubRadius * 2f
        val maxTapeRadius = hubRadius * 1.65f

        // Precision Clear Acrylic Window Slit (Narrow vertical slit matching White Spindle Drive Hub with 2px precision margin)
        ...
        // Spindle gap reduced by half:
        val defaultHubSpacing = centerWindowRect.height() * 0.28f
        val currentHubGap = (defaultHubSpacing - hubRadius) * 2f
        val targetHubGap = currentHubGap * 0.5f
        val hubSpacing = hubRadius + targetHubGap * 0.5f
        topHubCenter.set(reelCenterX, centerWindowRect.centerY() - hubSpacing)
        bottomHubCenter.set(reelCenterX, centerWindowRect.centerY() + hubSpacing)

        // Center align Nameplate & Time in the column between left frame (innerShellRect.left) and clear spindle frame (winLeft)
        val nameplateColLeft = innerShellRect.left
        val nameplateColRight = winLeft
        columnCenterX = (nameplateColLeft + nameplateColRight) * 0.5f
        textRightMarginX = columnCenterX

        // Primary & Secondary Acrylic Window Slit Specular Sheens
        glarePath1.reset()
        glarePath1.moveTo(centerWindowRect.left, centerWindowRect.top + centerWindowRect.height() * 0.35f)
        glarePath1.lineTo(centerWindowRect.left, centerWindowRect.top + centerWindowRect.height() * 0.55f)
        glarePath1.lineTo(centerWindowRect.right, centerWindowRect.top + centerWindowRect.height() * 0.30f)
        glarePath1.lineTo(centerWindowRect.right, centerWindowRect.top + centerWindowRect.height() * 0.10f)
        glarePath1.close()

        glarePath2.reset()
        glarePath2.moveTo(centerWindowRect.left, centerWindowRect.top + centerWindowRect.height() * 0.60f)
        glarePath2.lineTo(centerWindowRect.left, centerWindowRect.top + centerWindowRect.height() * 0.68f)
        glarePath2.lineTo(centerWindowRect.right, centerWindowRect.top + centerWindowRect.height() * 0.43f)
        glarePath2.lineTo(centerWindowRect.right, centerWindowRect.top + centerWindowRect.height() * 0.35f)
        glarePath2.close()

        // Tape Volume Ruler Graduations (Ticks along window slit)
        val rulerStep = (bottomHubCenter.y - topHubCenter.y) * 0.25f
        val midGapY = (topHubCenter.y + bottomHubCenter.y) * 0.5f
        for (i in 0 until 5) {
            rulerTickYs[i] = midGapY + (i - 2) * rulerStep
        }
        rulerTextPaint.textSize = winW * 0.15f

        // Studio Paper Label (Left Column / Spine) - reduced width with elegant left & right spacing
        val colLeft = cassetteRect.left
        val colRight = centerWindowRect.left
        val colWidth = colRight - colLeft
        val labelMarginH = colWidth * 0.15f
        val labelLeft = colLeft + labelMarginH
        val labelRight = colRight - labelMarginH
        val labelTop = cassetteRect.top + ch * 0.07f
        val labelBottom = cassetteRect.bottom - ch * 0.07f
        ...
        // Top Accent Stripe across label (Real-Time Clock Display)
        val stripeH = ch * 0.048f
        labelStripeRect.set(labelLeft, labelTop, labelRight, labelTop + stripeH)
        labelStripeTextPaint.textSize = stripeH * 0.52f

        // Album Art Badge (Square thumbnail with rounded corners below the clock stripe)
        val lw = labelRight - labelLeft
        val artSize = lw * 0.76f
        val artLeft = labelLeft + (lw - artSize) * 0.5f
        val artTop = labelStripeRect.bottom + ch * 0.016f
        val artRight = artLeft + artSize
        val artBottom = artTop + artSize
        labelSideBadgeRect.set(artLeft, artTop, artRight, artBottom)

        val artCornerR = artSize * 0.12f
        albumArtClipPath.reset()
        albumArtClipPath.addRoundRect(labelSideBadgeRect, artCornerR, artCornerR, Path.Direction.CW)

        labelSubTextPaint.textSize = lw * 0.14f
        ...
        rollerRadius = cw * 0.024f
        rollerX = cassetteRect.right - cw * 0.082f
        rollerYTop = trapTopOuterY + ch * 0.065f
        rollerYBottom = trapBottomOuterY - ch * 0.065f

        // Pre-compute Mumetal magnetic shielding plate inside head cavity
        val shieldW = headCavityRect.width() * 0.44f
        val shieldH = headCavityRect.height() * 0.68f
        magneticShieldRect.set(
            headCavityRect.left + 3f,
            headCavityRect.centerY() - shieldH * 0.5f,
            headCavityRect.left + 3f + shieldW,
            headCavityRect.centerY() + shieldH * 0.5f
        )
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        // 3. Draw Transparent Acrylic Window Slit & Kinetic Rotating Reels
        drawWindowPanelAndKineticReels(canvas)

        // 3b. Draw Threaded Magnetic Tape Path Across Rollers & Magnetic Shield Plate
        drawTapePathAndRollers(canvas)

        // 3c. Draw Cassette Structural Blueprint Wireframe Overlay (on top of reels & window)
        drawCassetteWireframeOverlay(canvas)

        // 4. Draw Expansive Studio Paper Label with Now Playing Typography (Rotated -90° Vertical)
        drawStudioPaperLabel(canvas)

        // 5. Draw 12-LED Song Playback Progress Bar
        drawLedProgressBar(canvas)

        // 6. Draw 4 Mechanical Tactile Buttons (REW, FWD, PLAY [Muted when playing], EJECT)
    ...
    private fun drawCassetteSlotAndShell(canvas: Canvas) {
        ...
        // 1. Tape head opening & Mumetal magnetic shield plate mechanism
        ...
        // 1b. Mumetal magnetic shield plate mounted behind spring and tape path
        canvas.drawRoundRect(magneticShieldRect, 3f, 3f, magneticShieldPlatePaint)
        canvas.drawRoundRect(magneticShieldRect, 3f, 3f, magneticShieldBorderPaint)
    ...
    /**
     * Draws the threaded magnetic tape ribbon extending from the outer tangent of the top reel,
     * wrapping around the precision guide rollers, passing across the Mumetal magnetic shield plate
     * and felt pad, and winding onto the outer tangent of the bottom reel.
     */
    private fun drawTapePathAndRollers(canvas: Canvas) {
        val cw = cassetteRect.width()
        val headH = headCavityRect.height() * 0.46f

        // 1. Two Precision Flanged Guide Rollers (flanking top & bottom of head opening)
        drawGuideRoller(canvas, rollerX, rollerYTop, rollerRadius, topReelAngle * 1.8f)
        drawGuideRoller(canvas, rollerX, rollerYBottom, rollerRadius, bottomReelAngle * 1.8f)

        // 2. Tangent Calculation: Tape departs OUTSIDE of top spool and enters OUTSIDE of bottom spool
        val spoolState = kinematics.calculate(progress, baseHubDimension)
        val rTop = spoolState.leftRadius
        val rBottom = spoolState.rightRadius
        kinematics.calculateSpoolToRollerTangent(
            topHubCenter.x, topHubCenter.y, rTop,
            rollerX, rollerYTop, rollerRadius,
            true, topTangentPoints
        )
        kinematics.calculateSpoolToRollerTangent(
            bottomHubCenter.x, bottomHubCenter.y, rBottom,
            rollerX, rollerYBottom, rollerRadius,
            false, bottomTangentPoints
        )

        // 3. Thread magnetic tape cleanly along outer path across magnetic plate & head
        threadedTapePath.reset()
        // Start on outer tangent of top supply reel
        threadedTapePath.moveTo(topTangentPoints[0], topTangentPoints[1])
        // Travel to top guide roller
        threadedTapePath.lineTo(topTangentPoints[2], topTangentPoints[3])
        threadedTapePath.lineTo(rollerX + rollerRadius, rollerYTop)

        // Pass across magnetic shielding plate and tape head
        val retractedHeadX = headCavityRect.right + 12f
        val engagedHeadX = pressurePadRect.right + 2f
        val currentHeadX = retractedHeadX + (engagedHeadX - retractedHeadX) * headEngageProgress
        if (headEngageProgress > 0.05f) {
            val tapeTouchX = currentHeadX.coerceAtMost(rollerX + rollerRadius)
            threadedTapePath.lineTo(tapeTouchX, headCavityRect.centerY() - headH * 0.38f)
            threadedTapePath.lineTo(tapeTouchX, headCavityRect.centerY() + headH * 0.38f)
        } else {
            threadedTapePath.lineTo(rollerX + rollerRadius, headCavityRect.centerY())
        }

        // Travel to bottom guide roller
        threadedTapePath.lineTo(rollerX + rollerRadius, rollerYBottom)
        threadedTapePath.lineTo(bottomTangentPoints[2], bottomTangentPoints[3])
        // Enter outer tangent of bottom take-up reel
        threadedTapePath.lineTo(bottomTangentPoints[0], bottomTangentPoints[1])

        // 4. Draw magnetic tape ribbon (or clear polyfilm leader tape)
        val isLeaderHead = (progress <= 0.025f)
        val isLeaderTail = (progress >= 0.975f)

        if (isLeaderHead || isLeaderTail) {
            canvas.drawPath(threadedTapePath, leaderTapePaint)
            canvas.drawPath(threadedTapePath, leaderTapeHighlightPaint)

            // Dynamic diagonal amber splice tape indicator
            val spliceRatio = if (isLeaderHead) (progress / 0.025f) else ((1.0f - progress) / 0.025f)
            val spliceY = rollerYTop + (rollerYBottom - rollerYTop) * spliceRatio
            val tapeX = rollerX + rollerRadius
            canvas.drawLine(tapeX - 5.5f, spliceY - 4.5f, tapeX + 5.5f, spliceY + 4.5f, tapeSplicePaint)
        } else {
            canvas.drawPath(threadedTapePath, tapePathPaint)
            canvas.drawPath(threadedTapePath, tapePathHighlightPaint)
        }

        // 5. Animated traveling tape surface motion sheen along outer ribbon run
        val tapeSpanH = rollerYBottom - rollerYTop
        if (tapeSpanH > 0f && isPlaying) {
            val step = 20f
            val count = (tapeSpanH / step).toInt() + 1
            val tapeX = rollerX + rollerRadius
            for (i in 0 until count) {
                val sheenY = rollerYTop + ((i * step + tapeTravelOffset * 2.5f) % tapeSpanH)
                canvas.drawLine(tapeX - 3.5f, sheenY, tapeX + 3.5f, sheenY, tapeMotionSheenPaint)
            }
        }
    }
    ...
    private fun drawCassetteWireframeOverlay(canvas: Canvas) {
        ...
        // 5. Molded Concentric Reel Wells on Cassette Shell (Embossed ABS plastic contour rings)
        val maxTapeR = hubOuterRadius * 1.65f
        canvas.drawCircle(topHubCenter.x, topHubCenter.y, maxTapeR, innerBevelPaint)
        canvas.drawCircle(topHubCenter.x, topHubCenter.y, maxTapeR * 0.82f, innerBevelPaint)
        canvas.drawCircle(topHubCenter.x, topHubCenter.y, hubOuterRadius * 1.30f, cassetteSubtleOutlinePaint)

        canvas.drawCircle(bottomHubCenter.x, bottomHubCenter.y, maxTapeR, innerBevelPaint)
        canvas.drawCircle(bottomHubCenter.x, bottomHubCenter.y, maxTapeR * 0.82f, innerBevelPaint)
        ...
        // 9. Capstan Drive Holes (circular molded openings for deck drive pins aligned with spindle centers)
        val capstanR = cw * 0.024f
        val capstanX = cassetteRect.left + cw * 0.910f
        val capstanYTop = topHubCenter.y
        val capstanYBottom = bottomHubCenter.y
        ...
        val prYTop = (topHubCenter.y + midY) * 0.5f
        val prYBottom = (bottomHubCenter.y + midY) * 0.5f
        ...
        if (isSony || isTdk || isMaxell || isBasf) {
            ...
            foilBadgePaint.shader = LinearGradient(
                ...
                intArrayOf(foilColor1, foilColor2, foilColor1),
                null, Shader.TileMode.CLAMP
            )
            canvas.drawText(foilBrand, foilX, foilY, foilBadgePaint)
        }
    }

    /**
     * Draws a precision flanged guide roller flanking the head cavity opening.
     */
    private fun drawGuideRoller(canvas: Canvas, cx: Float, cy: Float, radius: Float, rotationAngle: Float) {
        // 1. Recessed circular well
        canvas.drawCircle(cx, cy, radius * 1.15f, guideRollerWellPaint)

        // 2. Flanged outer guide rim
        canvas.drawCircle(cx, cy, radius, guideRollerRimPaint)

        // 3. Delrin roller body
        canvas.drawCircle(cx, cy, radius * 0.82f, guideRollerBodyPaint)

        // 4. Center axle pin / pivot boss
        canvas.drawCircle(cx, cy, radius * 0.32f, guideRollerPinPaint)

        // 5. Kinetic rotation strobe markers to visually convey tape motion
        val rad = Math.toRadians(rotationAngle.toDouble())
        val dotDist = radius * 0.58f
        val dotR = radius * 0.16f
        val cosA = kotlin.math.cos(rad).toFloat()
        val sinA = kotlin.math.sin(rad).toFloat()
        canvas.drawCircle(cx + dotDist * cosA, cy + dotDist * sinA, dotR, guideRollerStrobePaint)
        canvas.drawCircle(cx - dotDist * cosA, cy - dotDist * sinA, dotR, guideRollerStrobePaint)
    }

    /**
     * Draws a tactile badge selector for cassette tape formulation or Dolby noise reduction.
     */
    private fun drawSelectorBadge(
        canvas: Canvas,
        rect: RectF,
        mainText: String,
        subText: String,
        ...
        val r = 4f
        canvas.drawRoundRect(rect, r, r, tapeBadgeBgPaint)
        canvas.drawRoundRect(rect, r, r, tapeBadgeBorderPaint)

        val fmMain = tapeBadgeTextPaint.fontMetrics
        val fmSub = tapeBadgeSubTextPaint.fontMetrics
        val mainCapH = -fmMain.ascent
        val subCapH = -fmSub.ascent
        val gap = 2f
        val totalH = mainCapH + subCapH + gap
        val textTop = rect.centerY() - totalH * 0.5f

        val mainY = textTop + mainCapH
        val subY = mainY + gap + subCapH

        val prevColor = tapeBadgeTextPaint.color
        if (!isActive) {
            tapeBadgeTextPaint.color = tapeBadgeSubTextPaint.color
        }
        canvas.drawText(mainText, rect.centerX(), mainY, tapeBadgeTextPaint)
        tapeBadgeTextPaint.color = prevColor

        canvas.drawText(subText, rect.centerX(), subY, tapeBadgeSubTextPaint)
    }

    /**
     * Draws the status row directly above the 12-LED progress bar:
     * - Left-aligned: Interactive selector badges (Tape Formulation & Dolby NR)
     * - Right-aligned: Dual deck status LEDs ("Battery" and "Run")
     */
    private fun drawStatusLeds(canvas: Canvas) {
        val isEink = (theme.id == CassetteTheme.MONOCHROME_EINK.id)

        // 1. Left-aligned Tactile Tape Formulation, Dolby Noise Reduction, and J-Card Selectors
        ...
        // 5. "Run" text on right of status led
        canvas.drawText("Run", activeLabelX, activeLabelY, statusLabelPaint)
    }

    /**
     * Draws the central transparent acrylic window slit and the kinetic rotating reels visible within it.
     */
    private fun drawWindowPanelAndKineticReels(canvas: Canvas) {
        ...
        // 2. Central precision window slit cavity
        val winRadius = centerWindowRect.width() * 0.08f
        canvas.drawRoundRect(centerWindowRect, winRadius, winRadius, centerWindowPaint)
        canvas.drawRoundRect(centerWindowRect, winRadius, winRadius, centerWindowBorderPaint)

        // 3. Crisp, fully opaque magnetic tape packs & hubs INSIDE the clear window
        canvas.save()
        canvas.clipRect(centerWindowRect)

        // Kinetic Reel Wobble offsets (micro-eccentricity)
        val topWobbleX = kinematics.getWobbleOffsetX(topReelAngle, 0.85f)
        val topWobbleY = kinematics.getWobbleOffsetY(topReelAngle, 0.85f)
        val bottomWobbleX = kinematics.getWobbleOffsetX(bottomReelAngle, 0.85f)
        val bottomWobbleY = kinematics.getWobbleOffsetY(bottomReelAngle, 0.85f)
        ...
        drawSpoolRotatingSheenAndSpokes(canvas, topHubCenter.x + topWobbleX, topHubCenter.y + topWobbleY, hubOuterRadius, rTopTape, topReelAngle)
        if (progress <= 0.03f && rTopTape > hubOuterRadius + 4f) {
            canvas.drawCircle(topHubCenter.x, topHubCenter.y, rTopTape, leaderSpoolRingPaint)
        }
        ...
        drawSpoolRotatingSheenAndSpokes(canvas, bottomHubCenter.x + bottomWobbleX, bottomHubCenter.y + bottomWobbleY, hubOuterRadius, rBottomTape, bottomReelAngle)
        if (progress >= 0.97f && rBottomTape > hubOuterRadius + 4f) {
            canvas.drawCircle(bottomHubCenter.x, bottomHubCenter.y, rBottomTape, leaderSpoolRingPaint)
        }

        // Top Reel Hub Mechanism (White Delrin Spindle Drive Hub with kinetic wobble)
        drawHubMechanism(canvas, topHubCenter.x + topWobbleX, topHubCenter.y + topWobbleY, hubOuterRadius, topReelAngle)

        // Bottom Reel Hub Mechanism (White Delrin Spindle Drive Hub with kinetic wobble)
        drawHubMechanism(canvas, bottomHubCenter.x + bottomWobbleX, bottomHubCenter.y + bottomWobbleY, hubOuterRadius, bottomReelAngle)

        // 4. Slit Graduation Scale (Ticks along window slit edges)
        val tickLen = 5.5f
        for (t in 0 until 5) {
            val ty = rulerTickYs[t]
            if (ty < bottomHubCenter.y - hubOuterRadius - 6f && ty > topHubCenter.y + hubOuterRadius + 6f) {
                canvas.drawLine(centerWindowRect.left + 2f, ty, centerWindowRect.left + 2f + tickLen, ty, rulerTickPaint)
                canvas.drawLine(centerWindowRect.right - 2f - tickLen, ty, centerWindowRect.right - 2f, ty, rulerTickPaint)
            }
        }

        // Center "— 50 —" Midpoint Calibration Marker
        val midGapY = (topHubCenter.y + bottomHubCenter.y) * 0.5f
        val midTickW = 14f
        canvas.drawLine(topHubCenter.x - midTickW - 8f, midGapY, topHubCenter.x - 8f, midGapY, rulerTickPaint)
        canvas.drawText("50", topHubCenter.x, midGapY + (rulerTextPaint.textSize * 0.35f), rulerTextPaint)
        canvas.drawLine(topHubCenter.x + 8f, midGapY, topHubCenter.x + midTickW + 8f, midGapY, rulerTickPaint)

        // Diagonal Acrylic Window Specular Glints
        canvas.drawPath(glarePath1, glarePaint)
        canvas.drawPath(glarePath2, glarePaint2)

        canvas.restore()

        // Subtle specular highlight on top of acrylic window
        tempRectF.set(
            centerWindowRect.left + 3f,
            centerWindowRect.top + 3f,
            centerWindowRect.right - 3f,
            centerWindowRect.top + 20f
        )
        canvas.drawRoundRect(tempRectF, winRadius * 0.6f, winRadius * 0.6f, windowGlassHighlightPaint)
    }
    ...
    /**
     * Draws the authentic expansive studio cassette paper label (Left Spine).
     *
     * Hierarchy:
     * 1. Recessed Label Well (Outer chamfer / shadow)
     * 2. Studio Paper Base (Theme-aware card stock) with subtle perimeter border
     * 3. Top Formulation Stripe with high-contrast text ("TYPE II", "NORMAL", "METAL")
     * 4. Side "A" Badge Pill
     * 5. Ruled paper guide lines
     * 6. Rotated -90° 3-Row Studio Typography:
     *    - Row 1: Track Title (Bold studio typography, auto-sizing / marquee scrolling when playing)
     *    - Row 2: Artist Name • Playback Time (or real-time synchronized lyrics ticker)
     *    - Row 3: Audio Specs • Output Routing (or Hardware / OS in idle mode)
     */
    private fun drawStudioPaperLabel(canvas: Canvas) {
        if (labelRect.width() <= 0f || labelRect.height() <= 0f) return

        // 1. Recessed Label Well & Paper Base
        val labelRadius = width.toFloat() * 0.015f
        ...
        // 2. Real-Time Clock Accent Stripe (Live system clock)
        canvas.drawRect(labelStripeRect, labelStripePaint)
        val now = System.currentTimeMillis()
        calendar.timeInMillis = now
        val hour = calendar.get(Calendar.HOUR_OF_DAY)
        val minute = calendar.get(Calendar.MINUTE)
        val clockText = String.format(Locale.US, "%02d:%02d", hour, minute)
        canvas.drawText(
            clockText,
        ...
        // 3. Album Art Badge (Album Artwork Thumbnail or Stylized Studio Emblem)
        val artRadius = labelSideBadgeRect.height() * 0.12f
        val artBitmap = albumArtBitmap
        if (artBitmap != null && !artBitmap.isRecycled) {
            canvas.save()
            canvas.clipPath(albumArtClipPath)
            albumArtSrcRect.set(0, 0, artBitmap.width, artBitmap.height)
            canvas.drawBitmap(artBitmap, albumArtSrcRect, labelSideBadgeRect, null)
            canvas.restore()
            canvas.drawRoundRect(labelSideBadgeRect, artRadius, artRadius, albumArtBorderPaint)
        } else {
            // Elegant studio vinyl emblem placeholder
            canvas.drawRoundRect(labelSideBadgeRect, artRadius, artRadius, albumArtPlaceholderPaint)
            canvas.drawRoundRect(labelSideBadgeRect, artRadius, artRadius, albumArtBorderPaint)
            val artCx = labelSideBadgeRect.centerX()
            val artCy = labelSideBadgeRect.centerY()
            val artR = labelSideBadgeRect.width() * 0.36f
            canvas.drawCircle(artCx, artCy, artR, albumArtIconPaint)
            canvas.drawCircle(artCx, artCy, artR * 0.35f, albumArtPlaceholderPaint)
            canvas.drawCircle(artCx, artCy, artR * 0.14f, albumArtIconPaint)
        }

        // 4. Vertical Ruled Paper Lines & Spine Title Typography (-90° rotation)
        canvas.save()
        ...
        // --- ROW 1: Track Title (Bold Studio Typography) ---
        val displayTitle = if (hasSong) {
            trackTitle
        } else if (deviceName.isNotBlank()) {
            deviceName
        } else {
            com.hana.spindle.util.DeviceUtils.getHardwareDeviceName()
        }

        val measuredTitleW = labelTitleTextPaint.measureText(displayTitle)
        val titleY = -lw * 0.16f

        if (hasSong && isPlaying && measuredTitleW > spineLen) {
            ...
            canvas.save()
            tempRectF.set(-spineLen * 0.5f, -lw * 0.35f, spineLen * 0.5f, ruleY1)
            canvas.clipRect(tempRectF)
            labelTitleTextPaint.textAlign = Paint.Align.LEFT
            val startX = -spineLen * 0.5f - marqueeOffset
            canvas.drawText(displayTitle, startX, titleY, labelTitleTextPaint)
            canvas.drawText(displayTitle, startX + measuredTitleW + 48f, titleY, labelTitleTextPaint)
            labelTitleTextPaint.textAlign = Paint.Align.CENTER
            canvas.restore()
        } else if (measuredTitleW > spineLen) {
            val budget = spineLen - labelTitleTextPaint.measureText("…")
            var trimmed = displayTitle
            while (trimmed.isNotEmpty() && labelTitleTextPaint.measureText(trimmed) > budget) {
                trimmed = trimmed.dropLast(1)
            }
            canvas.drawText("${trimmed.trimEnd()}…", 0f, titleY, labelTitleTextPaint)
        } else {
            canvas.drawText(displayTitle, 0f, titleY, labelTitleTextPaint)
        }

        // --- ROW 2: Artist • Playback Duration / Synchronized Lyrics ---
        val isShowingLyrics = hasSong && isLyricsModeEnabled && currentLyricText.isNotBlank()
        val displayArtistTime = if (isShowingLyrics) {
            currentLyricText
        } else if (hasSong) {
            val timeStr = formatPlaybackTime(currentTimeMs, durationMs)
            val artistPart = if (artistName.isNotBlank() && artistName != "Unknown Artist") artistName else "Unknown Artist"
            "$artistPart • $timeStr"
        } else {
            "STUDIO MASTER • STEREO"
        }

        val origSubColor = labelSubTextPaint.color
        if (isShowingLyrics) {
            labelSubTextPaint.color = theme.vfdGlowColor
        }
        val measuredSubW = labelSubTextPaint.measureText(displayArtistTime)
        val subY = lw * 0.07f
        if (measuredSubW > spineLen) {
            val budget = spineLen - labelSubTextPaint.measureText("…")
            var trimmed = displayArtistTime
            while (trimmed.isNotEmpty() && labelSubTextPaint.measureText(trimmed) > budget) {
                trimmed = trimmed.dropLast(1)
            }
            canvas.drawText("${trimmed.trimEnd()}…", 0f, subY, labelSubTextPaint)
        } else {
            canvas.drawText(displayArtistTime, 0f, subY, labelSubTextPaint)
        }
        labelSubTextPaint.color = origSubColor

        // --- ROW 3: Audio Specs • Output Routing / System Info ---
        val displaySpecs = if (hasSong) {
            val specs = if (audioFormat.isNotBlank()) audioFormat else "HI-RES AUDIO"
            val route = getRouteRowText()
            "$specs • $route"
        } else {
        ...
        val measuredSpecsW = labelSpecsTextPaint.measureText(displaySpecs)
        val specsY = lw * 0.31f
        if (measuredSpecsW > spineLen) {
            val budget = spineLen - labelSpecsTextPaint.measureText("…")
            var trimmed = displaySpecs
            while (trimmed.isNotEmpty() && labelSpecsTextPaint.measureText(trimmed) > budget) {
                trimmed = trimmed.dropLast(1)
            }
            canvas.drawText("${trimmed.trimEnd()}…", 0f, specsY, labelSpecsTextPaint)
        } else {
            canvas.drawText(displaySpecs, 0f, specsY, labelSpecsTextPaint)
        }
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        when (event.actionMasked) {
            MotionEvent.ACTION_DOWN -> {
                ...
                // Check Dolby NR selector badge touch
                val inDolbyBadge = x >= dolbyBadgeRect.left - badgeSlop &&
                                   x <= dolbyBadgeRect.right + badgeSlop &&
                                   y >= dolbyBadgeRect.top - badgeSlop &&
                                   y <= dolbyBadgeRect.bottom + badgeSlop

                if (inDolbyBadge) {
                    performHapticFeedback(HapticFeedbackConstants.KEYBOARD_TAP)
                    val nextDolby = when (dolbyMode) {
                        com.hana.spindle.playback.AudioFxController.DolbyMode.OFF ->
                            com.hana.spindle.playback.AudioFxController.DolbyMode.DOLBY_B
                        com.hana.spindle.playback.AudioFxController.DolbyMode.DOLBY_B ->
                            com.hana.spindle.playback.AudioFxController.DolbyMode.DOLBY_C
                        com.hana.spindle.playback.AudioFxController.DolbyMode.DOLBY_C ->
                            com.hana.spindle.playback.AudioFxController.DolbyMode.OFF
                    }
                    dolbyMode = nextDolby
                    onDolbyClicked?.invoke(nextDolby)
                    invalidate()
                    return true
                }

                // Check 4 bottom buttons
                val inJCardBadge = x >= jCardBadgeRect.left - badgeSlop &&
                                   x <= jCardBadgeRect.right + badgeSlop &&
                                   y >= jCardBadgeRect.top - badgeSlop &&
                                   y <= jCardBadgeRect.bottom + badgeSlop
                if (inJCardBadge) {
                ...
                if (pressedButtonIndex != -1) {
                    ...
                    // If REW (0) or FWD (1), schedule hold-seeking runnable after 320ms
                    if (pressedButtonIndex == 0 || pressedButtonIndex == 1) {
                        val btnIndex = pressedButtonIndex
                        val isFwd = (btnIndex == 1)
                        ...
                        holdSeekRunnable = object : Runnable {
                            override fun run() {
                                if (!isHoldSeeking) {
                                    isHoldSeeking = true
                                    onHoldSeekStart?.invoke(isFwd)
                                }
                                seekTicks++
                                val speedMultiplier = (1.0f + seekTicks * 0.22f).coerceAtMost(6.5f)
                                val pitch = (0.92f + (speedMultiplier - 1.0f) * 0.12f).coerceIn(0.92f, 1.65f)
                                onHoldSeekProgress?.invoke(pitch)

                                if (isFwd) {
                                    onHoldSeekForward?.invoke()
                                    topReelAngle = (topReelAngle + 18f * speedMultiplier) % 360f
                                    bottomReelAngle = (bottomReelAngle + 18f * speedMultiplier) % 360f
                                    tapeTravelOffset = (tapeTravelOffset + 6f * speedMultiplier) % 40f
                                } else {
                                    onHoldSeekRewind?.invoke()
                                    topReelAngle = (topReelAngle - 18f * speedMultiplier + 360f) % 360f
                                    bottomReelAngle = (bottomReelAngle - 18f * speedMultiplier + 360f) % 360f
                                    tapeTravelOffset = (tapeTravelOffset - 6f * speedMultiplier + 40f) % 40f
                                }

                                // Boundary check: Auto-stop if reaching leader boundaries
                                if ((isFwd && progress >= 0.998f) || (!isFwd && progress <= 0.002f)) {
                                    onLeaderAutoStop?.invoke()
                                    gestureHandler.removeCallbacks(this)
                                    isHoldSeeking = false
                                ...
                                performHapticFeedback(HapticFeedbackConstants.KEYBOARD_TAP)
                                invalidate()
                                gestureHandler.postDelayed(this, 50L)
                            }
                        }
                        gestureHandler.postDelayed(holdSeekRunnable!!, 320L)
                    }
    ...
    private fun startRotation() {
        ...
        rotationAnimator = ValueAnimator.ofFloat(0f, 360f).apply {
            ...
            addUpdateListener {
                val timeSec = (SystemClock.uptimeMillis() % 60000L) / 1000f
                val wowFlutterMod = kinematics.getWowFlutterMultiplier(timeSec)
                val spoolState = kinematics.calculate(progress, baseHubDimension)
                topReelAngle = (topReelAngle + 4.5f * spoolState.leftAngularSpeed * wowFlutterMod) % 360f
                bottomReelAngle = (bottomReelAngle + 4.5f * spoolState.rightAngularSpeed * wowFlutterMod) % 360f
                tapeTravelOffset = (tapeTravelOffset + 2.8f * spoolState.leftAngularSpeed * wowFlutterMod) % 40f
                invalidate()
            }
            start()
        }
